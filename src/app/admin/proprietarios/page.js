...
import { getProprietarios } from "../services";
...
export default function AdminProprietarios() {
    ...
    const loadProprietarios = async (page = 1, search = "") => {
        setIsLoading(true);
        try {
            if (search) {
                // Implementar busca de proprietarios quando necessário
                const response = await fetch(`/api/search-proprietarios?q=${encodeURIComponent(search)}`);
                ...
                const response = await getProprietarios({}, page, 20);
                if (response && response.proprietarios) {
    ...
    const handleSearch = (e) => {
        e.preventDefault();
        setCurrentPage(1);
        loadProprietarios(1, searchTerm);
    };
    ...
    const handleEdit = (proprietarioId) => {
        router.push(`/admin/proprietarios/editar/${proprietarioId}`);
    };
            ...
            <div className="">
                <div className="mb-8">
                    <div className="flex justify-between items-center mb-4">
                        <h1 className="text-2xl font-bold text-gray-900">Gerenciamento de Proprietários</h1>
                        <button
                            onClick={() => router.push('/admin/proprietarios/cadastrar')}
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                {isLoading ? (
                                    ...
                                    proprietarios.map((proprietario) => (
                                        <tr key={proprietario._id} className="hover:bg-gray-50">
                                            <td className="px-6 py-4 whitespace-nowrap text-xs text-gray-900 font-bold capitalize">
                                                {proprietario.nomeCompleto || proprietario.nome}
                                            </td>
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap sticky right-0 bg-white">
                                                <div className="flex items-center space-x-3">
                                                    <button
                                                        ...
                                                        title="Editar"
                                                        onClick={() => handleEdit(proprietario.codigoC)}
                                                    >
