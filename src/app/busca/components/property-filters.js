...
export default function PropertyFilters({
  ...
  // 🔒 Injeção de CSS — trava pan horizontal no mobile (cirúrgico)
  useEffect(() => {
    if (typeof window === "undefined") return;
    const STYLE_ID = "pf-no-x-scroll-mobile";
    ...
    const css = `
      @media (max-width: 767px) {
        ...
        .overflow-x-auto, .overflow-x-scroll, .no-x-scroll {
          overflow-x: hidden !important;
  ...
  return (
    <>
      {/* Botão flutuante para abrir — mesma posição de antes */}
      {isClient && isMobile && !isVisible && (
        <button
          onClick={() => setIsVisible?.(true)}
          className="fixed bottom-20 left-1/2 -translate-x-1/2 z-[9990] px-6 py-3 rounded-full bg-black text-white shadow-xl"
